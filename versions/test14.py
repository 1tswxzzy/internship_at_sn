...
def analyze_district_data(ws, head_size, max_h, school_type, merge_map=None):
    ...
    def calc_stats(data_list, ind_key, multiplier, district_name, indicator_name):
        """计算并打印详细过程"""
        print(f"\n{'='*60}")
        print(f"📊 计算区县【{district_name}】-【{indicator_name}】")
        print(f"{'='*60}")
        print(f"{'学校名称':<30} {'学生数(A)':>10} {'原始指标值':>12} {'换算值(指标*乘数/A)':>20}")
        print("-" * 75)
        ...
        for d in data_list:
            ...
            ind_val = d[ind_key]
            A_sum += A
            indicator_sum += ind_val
            if A > 0:
                ratio = (ind_val * multiplier) / A
            else:
                ratio = 0.0
            ratios.append((A, ratio))
            print(f"{d['school']:<30} {A:>10.1f} {ind_val:>12.2f} {ratio:>20.4f}")

        print("-" * 75)
        print(f"  学生总数(A_sum) = {A_sum:.2f}")
        print(f"  指标总和(indicator_sum) = {indicator_sum:.2f}")
        ...
        for A, ratio in ratios:
            variance += (A / A_sum) * (ratio - X) ** 2

        S = np.sqrt(variance) if variance > 0 else 0.0
        CV = round((S / X), 2) if X != 0 else 0.0

        print(f"  ✨ 计算结果：")
        print(f"     加权平均值 X = {indicator_sum * multiplier:.2f} / {A_sum:.2f} = {X:.4f}")
    ...
    for dist, info in district_groups.items():
        ...
        for i, (name, mult) in enumerate(indicators):
            ind_key = f'ind{i+1}'
            stats = calc_stats(data, ind_key, mult, dist, name)  # 详细打印
    ...
    total_row = ['', '总计']
    for i, (name, mult) in enumerate(indicators):
        stats = calc_stats(all_data_lists[f'ind{i+1}'], f'ind{i+1}', mult, '总计', name)
