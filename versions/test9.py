...
def calc_media_per_stu(ws, head_size, max_h):
    ...
    for item in pending_items:
        print(f"\n发现不计校数的学校：{item['school']} (行{item['row']})")
        while True:
            ...
            if master_name in school_index:
                ...
                print(f"已将数据合并到 {master_name}，更新后：在校生数={new_stu}, 多媒体教室={new_media}, 每百名={new_result}")
                break
            else:
    ...
    for item in pending_items:
        ...
        while True:
            ...
            if master_name in school_index:
                ...
                rows[idx][3] = new_edu
                rows[idx][4] = new_result
                print(f"已合并到 {master_name}，更新：学生={new_stu}, 学历教师={new_edu}, 每百名={new_result}")
                break
            else:
    ...
    for item in pending_items:
        ...
        while True:
            ...
            if master_name in school_index:
                ...
                rows[idx][3] = new_back
                rows[idx][4] = new_result
                print(f"已合并到 {master_name}，更新：学生={new_stu}, 骨干教师={new_back}, 每百名={new_result}")
                break
            else:
                print(f"未找到学校“{master_name}”。")
    return rows
    ...
    for item in pending_items:
        ...
        while True:
            ...
            if master_name in school_index:
                ...
                print(f"已合并到 {master_name}，更新：学生={new_stu}, 体育={new_sport}, 艺术={new_art}, 每百名={new_result}")
                break
            else:
                print(f"未找到学校“{master_name}”。")
    return rows
...
def analyze_district_data(ws, head_size, max_h, school_type):
    ...
    print(f"[调试] district_col={district_col}, stu_col={stu_col}, edu_cols={edu_cols}, backbone_col={backbone_col}, sport_col={sport_col}, art_col={art_col}, count_col={count_col}")

    # 3. 先读取正常计校数的行，组成district_data；同时收集不计校数行
    district_data = OrderedDict()
    pending_district_items = []  # 存储不计校数的行数据，稍后合并
    ...
    # 4. 处理不计校数的行，合并到原校数据中
    # 先构建学校名到(district, index)的映射
    school_map = {}  # school_name -> (district, list_index)
    for dist, entries in district_data.items():
        for idx, entry in enumerate(entries):
            sname = entry['school']
            if sname:
                school_map[sname] = (dist, idx)

    for item in pending_district_items:
        print(f"\n[区县统计] 发现不计校数学校：{item['school']} (行{item['row']})，所属区县：{item['district']}")
        while True:
            master_name = input("请输入该校所属的原校点名称：").strip()
            if not master_name:
                continue
            if master_name in school_map:
                ...
                try:
                    add_stu = float(item['stu']) if item['stu'] is not None else 0
                    add_edu = sum(float(item['edu_vals'][c]) if item['edu_vals'][c] is not None else 0 for c in edu_cols)
                    add_back = float(item['back']) if item['back'] is not None else 0
                    add_sport = float(item['sport']) if item['sport'] is not None else 0
                    add_art = float(item['art']) if item['art'] is not None else 0
                except (ValueError, TypeError):
                    add_stu = add_edu = add_back = add_sport = add_art = 0

                entry['A'] += add_stu
                entry['B'] += add_edu
                entry['C'] += add_back
                entry['D'] += (add_sport + add_art)
                print(f"已合并到 {master_name} (区县: {dist})，更新后学生数: {entry['A']}")
                break
            else:
                print(f"未找到学校“{master_name}”，请重新输入。")
