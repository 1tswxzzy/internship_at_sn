...
def find_col_by_hierarchy(ws, head_size, hierarchy):
    """
    根据表头层级路径查找唯一列。
    hierarchy: ['关键词1', '关键词2', ...] ，要求这些关键词按行序从上到下出现在同一列中。
    返回：列号(int) 或 None
    """
...
def calc_media_per_stu(ws, head_size, max_h):
    ...
    # 若表头中“网络多媒体教室”没有子层级，也可用 find_col_by_keywords 保底
    if not media_col:
...
def calc_high_education_per_stu(ws, head_size, max_h, school_type):
    """每百名学生高学历教师数"""
    # 定位学历区域：路径为 专任教师 -> #按学历分
    edu_region = find_cols_with_prefix(ws, head_size, ['专任教师(按教授学生类型:小学、小学教学点、九年一贯制学校、十二年一贯制学校教小学生的专任教师)', '#按学历分'])
    if not edu_region:
        print("警告：未找到学历区域，尝试使用单个关键词查找。")
        # 降级方案：直接用关键词搜索
        ...
        for kw in keywords:
            ...
            for col in edu_region:
                if any(kw in str(ws.cell(row=r, column=col).value or '') for r in range(1, head_size+1)):
                    found = col
                    break
            if found:
...
def calc_art_sport_per_stu(ws, head_size, max_h):
    ...
    # 艺术下的“计”
    art_count_col = find_col_by_hierarchy(ws, head_size, ['专任教师', '#按授课课程分', '艺术', '计'])

    if not stu_col or (not sport_col and not art_count_col):
    ...
    for r in range(head_size + 1, max_h + 1):
        ...
        art_val = ws.cell(row=r, column=art_count_col).value if art_count_col else 0
        try:
            stu = float(stu_val) if stu_val is not None else None
