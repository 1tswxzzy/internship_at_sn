...
def calculate_section_ratios(all_special_schools, file_map):
    ...
    print("\n" + "=" * 60)
    print("第一步：计算特殊学校学段占比（优化版）")
    print("=" * 60)

    # ----- 预加载三个文件的学校->学生数映射 -----
    print("正在预加载各学段的学生数据...")
    school_student_cache = {}  # {'小学': {学校名: 学生数}, ...}
    for section, file_type in section_file_map.items():
        if file_type not in file_map:
            school_student_cache[section] = {}
            continue
        ...
        if stu_col:
            for r in range(head_size + 1, max_h + 1):
                ...
                if name:
                    ...
                    try:
                        stu = float(stu_val) if stu_val is not None else 0
                    except (ValueError, TypeError):
                        stu = 0
                    cache[name_str] = stu
        school_student_cache[section] = cache
        wb.close()
    print("预加载完成。\n")
    ...
    for special_type, schools in all_special_schools.items():
        ...
        for school in schools:
            ...
            for section in school_type_weights.keys():
                # 直接从缓存中获取学生数
                cache = school_student_cache.get(section, {})
                student_count = cache.get(school_name, 0)
            ...
            for section in school_type_weights.keys():
                if total_weighted > 0:
                    ratios[section] = (section_students.get(section, 0) *
                                       school_type_weights[section]) / total_weighted
                else:
                    ratios[section] = 0
